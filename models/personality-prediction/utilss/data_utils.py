...
from utilss.author_100recent import get_100_recent_posts
...
class MyMapDataset(Dataset):
    def __init__(self, dataset, tokenizer, token_length, DEVICE, mode):
        if dataset == 'essays':
            ...
            author_ids, input_ids, targets = dataset_processors.essays_embeddings(datafile, tokenizer, token_length, mode,False)
        elif dataset == 'kaggle':
            ...
            datafile = './test.csv'
            author_ids, input_ids, targets = dataset_processors.essays_embeddings(datafile, tokenizer, token_length, mode,True)
        if(dataset!='test'):
