...
def training(dataset,inputs, full_targets):
    ...
    for trait_idx in range(full_targets.shape[1]):
        ...
        max_val_acc = -1
        # model_filepath = 'model_' + trait_labels[trait_idx]
        i = 0
        for train_index, test_index in skf.split(inputs, targets):
            ...
            model_filepath = 'model_' + trait_labels[trait_idx]+'_'+str(i)
            i+=1
            ...
            # if(max_val_acc <= 100 * max(history.history['val_accuracy'])):
            #     max_val_acc = 100 * max(history.history['val_accuracy'])
            #     tf.keras.models.save_model(
            #         model,
            #         model_filepath,
            #         overwrite=True,
